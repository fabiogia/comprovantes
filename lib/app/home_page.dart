...
class _HomePageState extends State<HomePage> {
  ...
  AppBar buildAppBar(BuildContext context) {
    return AppBar(title: const Text("Comprovantes"), actions: [
      ...
      body: ListView.separated(
          itemBuilder: (BuildContext context, int index) {
            if (index == 0) {
              ...
              // Modelo "Avulso. Digitar agora"
              if (modelos[index - 1].id == 0) {
                return ListTile(
                  title: Text(modelos[index - 1].titulo),
                  subtitle: Text(modelos[index - 1].exemplo),
                  onTap: () => _avulsoDigitarAgora(modelos[index - 1]),
                  visualDensity:
              ...
              return Slidable(
                  ...
                  endActionPane: ActionPane(
                    ...
                    children: [
                      ...
                      SlidableAction(
                        onPressed: (BuildContext context) => {
                          ModeloComprovantePage.editar(
                              context, modelos[index - 1])
                        },
                  ...
                  child: ListTile(
                    title: Text(modelos[index - 1].titulo),
                    subtitle: Text(modelos[index - 1].exemplo),
                    onTap: () => _copiarExemplo(modelos[index - 1].exemplo),
                    visualDensity:
                        const VisualDensity(horizontal: 0, vertical: -4),
                  ));
            }
  ...
  _avulsoDigitarAgora(ModeloComprovante modelo) async {
    // modelos[index - 1]),_copiarExemplo(modelos[index - 1]),
    await InputDialog.show(context, 'O que foi pago?', 'Informe o que foi pago',
        onSuccess: (exemplo) {
      _copiarExemplo('$exemplo - ${modelo.hoje()}');
    }, onCancel: () {});
  }

  _copiarExemplo(String exemplo) {
    //ArgumentError.checkNotNull(modelo, "modelo");

    if (_sharedFile != null) {
      Share.shareFiles([_sharedFile!.path], subject: exemplo, text: exemplo);
    } else {
      Toast.show("Sem imagem para compartilhar",
  ...
  _excluirCache() {
    ...
    String nomeDiretorio =
        sharedFilePath.substring(0, sharedFilePath.lastIndexOf('/'));
