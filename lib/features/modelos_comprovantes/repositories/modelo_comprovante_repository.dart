...
class ModeloComprovanteRepository extends ChangeNotifier {
  ...
  Future<void> _add(String titulo, String formato) async {
    await db.transaction((txn) async {
      await txn.insert('modelo', {
          'titulo': titulo,
          'formato': formato
        });

      final incluido = (await txn.query(
        'modelo',
        orderBy: 'id',
        limit: 1
      )).first;
      ...
      _modelos = _modelos
        ..add(ModeloComprovante(
          int.parse(incluido['id'] as String),
  ...
  Future<void> _update(int id, String titulo, String formato) async {
    await db.transaction((txn) async {
      ...
      await txn.update('modelo', 
        {'titulo': titulo, 'formato': formato},
        where: 'id = ?',
        whereArgs: [id]);

      _modelos[indexEditar] = _modelos[indexEditar]
        .atualizar(titulo, formato);
