...
class ModelosComprovantes {
  static List<ModeloComprovante> filtrarModelos(
      List<ModeloComprovante> todosModelos, String filtro) {
    filtro = filtro.trim();
    ...
    if (filtro.isEmpty) {
      ...
      return todosModelos
          .where((e) => e.tituloParaBusca.contains(filtro))
          .toList();
    }
  }
}
...
class ModeloComprovante {
  ...
  String hoje() {
    ...
    return formatoYMD.format(DateTime.now());
  }
  ...
  String _substituirVariaveis() {
    ...
    return formato.replaceAll("{hoje}", hoje())
